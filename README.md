...
The game is a **Progressive Web App**: once hosted over HTTPS you can install it to
your home screen and it runs fullscreen like a native app on **iOS and Android** —
no app store, no laptop needed to test.

**Hosting — Cloudflare Pages (free, private repo OK):**
1. Sign in at [dash.cloudflare.com](https://dash.cloudflare.com) → **Workers & Pages** → **Create** → **Pages** → **Connect to Git**.
2. Authorize GitHub and pick `mukhan27/CollegePets`.
3. Set the **production branch** to `claude/pwa-cloudflare` (the branch this PWA lives on).
4. Build settings: **Framework preset → None**, **Build command → empty**, **Build output directory → `/`**. (No build step — it's a static site.)
5. **Save and Deploy.** You get a URL like `https://college-pets.pages.dev`.
...
**Iteration loop:** every push to `claude/pwa-cloudflare` triggers a Cloudflare build
(~1 min); just reload/reopen the app on your phone to get the latest. App code is
served network-first by the service worker, so an online reload always shows the
newest version (no cache-clearing); the 3D engine and models are cached for fast,
offline-friendly loads.

> App icons are generated by `node scripts/gen-icons.mjs` (no dependencies) into `icons/`.
