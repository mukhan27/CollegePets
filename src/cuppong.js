...
export function createCupPong(scene) {
  ...
  function throwSwipe(ux, uy) {
    ...
    const un = clamp(uy / (window.innerHeight * 0.75), 0.08, 1.0);
    const sn = clamp(ux / (window.innerHeight * 0.75), -0.6, 0.6);
  ...
  function physics(dt) {
    ...
    if (b.vy < 0 && b.y - R <= TTOP && Math.abs(b.x) < 1.6 && b.z > -4.1 && b.z < 4.1) {
      b.y = TTOP + R; b.vy = -b.vy * 0.85; b.vx *= 0.92; b.vz *= 0.92; b.bounced = true;
    }
